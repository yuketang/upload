...
class Upload {
    ...
    putFileUpload(filePath = '', key = '', cb) {
        ...
        return new Promise((resolve, reject) => {
            ...
            // 文件上传
            formUploader.putFile(this.uploadToken(), key, filePath, this.putExtra, (err, result) => {
                ...
                if (!err && !result.error) {
                    ...
                    if ((err && err.error) || (result && result.error)) {
                        let error = new Error(err ? err.error : result.error);
    ...
    putStreamUpload(stream = '', key, cb) {
        return new Promise((resolve, reject) => {

            let formUploader = new qiniu.form_up.FormUploader(this.qn_conf);
            // 文件流上传
            formUploader.putStream(this.uploadToken(), key, stream, this.putExtra, function (err, result) {
                if (!err && !result.error) {
                    let resp = {
                        'url': this.host + '/' + result.key,
                        'size': result.fsize,
